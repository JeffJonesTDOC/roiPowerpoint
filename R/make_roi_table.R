...
make_roi_table <- function(has_rx, post_period_length, program, final_cohort_size,price_of_program,pppm_changes, supply_cost, pooled_cohort_table2) {
  ...
  {
    if (has_rx == T) {
      final_roi_table = as.data.frame(matrix(nrow=6,ncol=post_period_length+1))
      ...
      final_roi_table = as.data.frame(matrix(nrow=2,ncol=post_period_length+1))
    ...
    for (i in 1:post_period_length) {
      if (post_period_length > 1) {final_roi_table_headers[i+1] = paste("Year ",i," (N=",as.numeric(pooled_cohort_table2[3,i+1]),")",sep="")}
  ...
  # Now, turn final_roi_table into a flextable
  {
    ...
    roi_flextable <- flextable::width(roi_flextable,width=(5/(post_period_length+1)))
