extract_sheet_data <- function (roi_sheet, program, study, data_summary_sheet, data_overview_sheet,
                                 HTN_population) {
  price_of_program = round(as.numeric(roi_sheet[which(roi_sheet[,
                                                                1] == "Price of Livongo"), 3][1]), 0)
  {
    if (program == "Hypertension") {
      if (study == "2YR" | study == "3YR" | study == "4YR") {
        if (HTN_population == "All") {
          roi_sheet = roi_sheet[1:which(roi_sheet[, 1] ==
                                          "DMHTN"), ]
        }
        if (HTN_population == "HTN & DM") {
          roi_sheet = roi_sheet[(which(roi_sheet[, 1] ==
                                         "DMHTN") + 3):which(roi_sheet[, 1] == "HTN only"),
          ]
        }
        if (HTN_population == "HTN only") {
          roi_sheet = roi_sheet[(which(roi_sheet[, 1] ==
                                         "HTN only") + 3):nrow(roi_sheet), ]
        }
      }
  ...
  }
  {
    ...
    if (program == "Diabetes") {
      min_activation_length = data_overview_sheet[which(data_overview_sheet[,
                                                                            1] == "joined_months"), 2]
    }
    ...
    client_name = data_overview_sheet$data_overview[which(data_overview_sheet$X1 ==
                                                            "company_name")]
    if (length(client_name) == 0) {
      client_name = data_overview_sheet$data_overview[which(data_overview_sheet[,
                                                                                1] == "company_name")]
    }
    if (length(client_name) == 0) {
